...
describe UrbanairshipWrapper do
  ...
  it 'send only ios push with one user' do
    Urbanairship.should_receive(:push).with({:device_tokens => ['1234'], :aps => {:alert=>'haha', :badge=>1}}).exactly(1).times()
  ...
  it 'send two ios push and one android push' do
    Urbanairship.should_receive(:push).with({:device_tokens => ['1234', '4567'], :aps => {:alert=>'haha', :badge=>1}}).exactly(1).times()
